...
import com.oasys.posasset.repository.TChangeRequestRepository;
import com.oasys.posasset.service.TChangerequestService;
...
@Service
@Log4j2
public class TChangereqServiceImpl implements TChangerequestService {

	@Autowired
	TChangeRequestRepository changereqrepo;
...
//		try {
//			
//			//Optional<ChangeRequestEntity> deptOptional = changereqrepo.findByChangereqApplnNo(requestDTO.getChangereqApplnNo());
...
//		    callWorkFlowChangerequestService(workflowStatusUpdateDto);
//		} catch (Exception e) {
//			log.info(":::Change Request workflow initaiate error:::" + e);
//		}
	...
	private void addSubCriteria(CriteriaBuilder cb, List<Predicate> list, PaginationRequestDTO filterRequestDTO,
		...
		if (Objects.nonNull(filterRequestDTO.getFilters().get("shopCode"))
			...
			String rating = String.valueOf(filterRequestDTO.getFilters().get("shopCode").toString());
			list.add(cb.equal(from.get("shopCode"), rating));
		}
		...
		if (Objects.nonNull(filterRequestDTO.getFilters().get("applnStatus"))
			...
			else if (filterRequestDTO.getFilters().get("applnStatus").toString().equals(ApplnStatus.APPROVED.name())) {
	...
	public GenericResponse updateApproval(ApprovalDTO approvalDto) {
		...
		if (changeEntity.isPresent()) {
			...
			if (approvalDto.getEvent().equals("APPROVED")
				...
				changerequestlog.setUserName(approvalDto.getUserName());
				changerequestlog.setAction(approvalDto.getAction());
				change.setChangereqStatus(ChangereqStatus.PENDING);
				changereqrepo.save(change);
				changereqlog.save(changerequestlog);
				workflowStatusUpdateDto.setEvent(approvalDto.getEvent());
				resp = Library.getSuccessfulResponse(approvalDto, ErrorCode.SUCCESS_RESPONSE.getErrorCode(),
			...
			if (approvalDto.getEntityType().equalsIgnoreCase("BREWERY")
				...
				workflowStatusUpdateDto.setSubModuleNameCode("CHANGE_REQUEST_AEC");
			}
			if (approvalDto.getEntityType().equalsIgnoreCase("DISTILLERY")) {
				workflowStatusUpdateDto.setSubModuleNameCode("CHANGE_REQUEST_DISTILLERY");
			}
	...
	public GenericResponse update(ChangereqRequestDTO requestDTO) {
		try {
			...
			try {
				...
				workflowStatusUpdateDto.setEvent(requestDTO.getEvent());
				workflowStatusUpdateDto.setLevel(requestDTO.getLevel());
				workFlowService.callWorkFlowChangerequestService(workflowStatusUpdateDto);
			} catch (Exception e) {
	...
	public GenericResponse changereqCount(ChangereqCountRequestDTO requestDto) {
		...
		ticketStatusList.forEach(ticketStatus -> {
		    ...
		    appcount = changereqrepo.getAppCountByStatusAndDateRangeAndLicenseNumber(
		            ticketStatus.getId(), fromDate, toDate, licnumber, shopCode);
	...
	public String callWorkFlowChangerequestService(WorkflowDTO workflowStatusUpdateDto) {
		log.info(":::::WORKFLOW:::");
//		Optional<ChangeRequestEntity> deptOptional = changereqrepo.findByChangereqApplnNoIgnoreCase(workflowStatusUpdateDto.getApplicationNumber());
...
//			String applnno=deptOptional.get().getChangereqApplnNo();
			...
			workflowDto.setCallbackURL(domain + "/helpdesk/changerequesttest/updateWorkFlow");// this is call back URL to update the
...
//		log.info(":::::Change requestserviceEntry Update currently work with::::::updateprocess");
//		log.info(":::::Change requestservice workflow response::::::" + workflowStatusUpdateDto);
//		Boolean isSuccess = true;
//		log.info("::::::APPLICATIONNO::::::"+ workflowStatusUpdateDto.getApplicationNumber());	
//		try {
//			log.info(":::2SECONDS:::");
//			Thread.sleep(2000);
//			Optional<ChangeRequestEntity> deptOptional = changereqrepo.findByChangereqApplnNoIgnoreCase(workflowStatusUpdateDto.getApplicationNumber().trim());
//			if(deptOptional.isPresent()) {
...
//			}
//			else {
//				log.info("No ChangeRequestEntity found for application number: " + workflowStatusUpdateDto.getApplicationNumber());	
//			}
//			
//		} catch (Exception exception) {
//			log.error("===========error while updaing updateWorkFlowDetails data change request===", exception.fillInStackTrace());
//			isSuccess = false;
//		}
//		return isSuccess;
//	}
